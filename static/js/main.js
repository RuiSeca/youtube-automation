...
function initDashboardUpdates() {
    ...
    fetch("/status")
      ...
      .then((data) => {
        ...
        if (document.getElementById("videos-today")) {
          document.getElementById("videos-today").textContent =
          ...
          if (data.videos && data.videos.length > 0) {
            ...
            document.querySelectorAll(".upload-video").forEach((btn) => {
              btn.addEventListener("click", (e) => {
                ...
                if (
                  confirm(
                    `Are you sure you want to upload "${videoTitle}" to YouTube?`
                  )
                ) {
  ...
  function uploadVideo(videoPath, videoTitle) {
    showToast(
      "info",
      "Upload Started",
      `Starting upload of "${videoTitle}" to YouTube...`
    );
      ...
      .then((data) => {
        if (data.success) {
          showToast(
            "success",
            "Upload Success",
            `"${videoTitle}" has been uploaded to YouTube!`
          );
        } else {
          showToast(
            "error",
        ...
        .then((data) => {
          if (data.success) {
            showToast(
              "success",
              "Job Started",
              `New automation job "${formData.get("niche")}" has been started.`
            );
          } else {
            showToast(
              "error",
